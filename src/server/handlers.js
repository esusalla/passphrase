...
export function joinGame(player, games) {
	...
	for (const [otherPlayerName, otherPlayer] of game.players) {
		if (otherPlayerName !== player.name) otherPlayer.send(JSON.stringify({ teamOne: game.teamOne, teamTwo: game.teamTwo, }));
	}
}
...
export function startRound(player, game) {
	...
	// Send above information in addition to current word and reset elapsed round time and skips available to active player 
...
export function passToNextPlayer(player, game) {
	...
	for (const [otherPlayerName, otherPlayer] of game.players) {
		if (otherPlayerName !== activePlayer.name) otherPlayer.send(JSON.stringify({ lastWord: game.lastWord, playerOrder: game.playerOrder, }));
	}
